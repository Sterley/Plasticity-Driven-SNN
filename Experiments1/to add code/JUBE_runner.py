...
class JUBERunner():
    ...
    def run(self, trajectory, generation):
        ...
        path_ready = os.path.join(
            self.work_paths["ready_files"], "ready_%d_" % generation)
        self.prepare_run_file(path_ready)
        # Remove previous trajectories
        traj_path = os.path.join(self.path, "trajectories/")
        if os.path.isdir(traj_path):
            ...
            # grab last two generations if there is more than 2 trajectories
            # otherwise grab the latest one
            if len(traj_ids) > 1:
                previous_gens = [traj_ids[-2], traj_ids[-1]]
            elif len(traj_ids) == 1:
                previous_gens = [traj_ids[-1]]
            # only delete generations before last two versions
            for f in os.listdir(traj_path):
                gen_id = func(f)
                if gen_id not in previous_gens:
                    os.remove(os.path.join(traj_path, f))
        else:
            print('creating trajectories folder at {}'.format(traj_path))
            os.mkdir(traj_path)
    ...
    def run_bkp(self, trajectory, generation):
        """
        Takes care of running the generation by preparing the JUBE configuration files and, waiting for the execution
        by JUBE and gathering the results.
        This is the main function of the JUBE_runner
