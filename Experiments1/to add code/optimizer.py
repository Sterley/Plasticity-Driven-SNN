...
class GeneticAlgorithmOptimizer(Optimizer):
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        best_inds = tools.selBest(self.eval_pop_inds, 1)
        ...
        best_individuals = tools.selBest(self.hall_of_fame, 2)
        # ------- Create the next generation by crossover and mutation -------- #
        if self.g < NGEN - 1:  # not necessary for the last generation
            # Select the next generation individuals
            #CJR offspring = self.toolbox.select(self.pop, len(self.pop))
            preselection = self.toolbox.select(self.pop, len(self.pop) - 0) # -4,  - 3
            offspring = preselection # + best_individuals
            ...
            self.pop[:] = offspring #+ best_individuals
